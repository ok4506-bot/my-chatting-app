...
# AI에게 부여할 성격(시스템 프롬프트)을 말투별로 미리 준비해둡니다.
# 사이드바에서 고른 말투에 맞는 문구가 아래에서 골라져 사용됩니다.
TONE_PROMPTS = {
    "친절한 선생님": "너는 따뜻하고 친절한 데이터 분석 선생님이야. 항상 다정하고 쉬운 말로 차근차근 설명해줘. 반드시 순수 한국어로만 답해",
    "시크한 전문가": "너는 데이터 분석에 능숙한 시크한 전문가야. 군더더기 없이 핵심만 간결하고 담백하게 답해줘. 반드시 순수 한국어로만 답해",
    "귀여운 친구": "너는 귀엽고 발랄한 친구 같은 데이터 분석 도우미야. 이모티콘도 섞어가며 친근하고 밝은 말투로 답해줘. 반드시 순수 한국어로만 답해",
}
...
# -----------------------------------------------------------------------
# 3-1) 사이드바 만들기: 말투 고르기 + 대화 지우기 버튼
# -----------------------------------------------------------------------
with st.sidebar:
    st.header("⚙️ 설정")

    # st.radio는 선택값을 st.session_state.tone에 자동으로 저장해줍니다.
    # 여기서 고른 말투는 아래 5-2)에서 매번 새로 시스템 프롬프트를 만들 때
    # 바로바로 반영되므로, 대화 중간에 바꿔도 "다음 답부터" 즉시 적용됩니다.
    st.radio(
        "말투 고르기",
    ...
    # 버튼을 누르면 세션에 쌓인 대화 기록을 모두 비웁니다.
    if st.button("🗑️ 대화 지우기"):
        st.session_state.messages = []
        st.rerun()  # 화면을 새로고침해서 비워진 대화창을 바로 보여줍니다.

# 현재 선택된 말투에 맞는 시스템 프롬프트를 가져옵니다.
SYSTEM_PROMPT = TONE_PROMPTS[st.session_state.tone]
